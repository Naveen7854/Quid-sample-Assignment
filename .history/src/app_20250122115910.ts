...
app.post('/register', async (request, reply) => {
  ...
  try {
    ...
    const authInsertQuery = `INSERT INTO userauth (email, password) VALUES ($1, $2) RETURNING id`;
    const authResult = await client.query(authInsertQuery, [email, hashedPassword]);
   // const insertQuery = `INSERT INTO userauth (email, password) VALUES ($1, $2)`;
    // Insert into userdata table for tracking
    const userInsertQuery = `INSERT INTO userdata (email) VALUES ($1)`;
    await client.query(userInsertQuery, [email]);

    client.release();
    reply.code(201).send({ message: 'User registered successfully', userId: authResult.rows[0].id });
  } catch (error) {
