...
//     const client = await app.pg.connect();
//     const selectQuery = `SELECT id, email FROM userdata WHERE id = ?`;
...
app.get('/getuser', async (request, reply) => {
  ...
  try {
    const secretKey = proces
    s.env.JWT_SECRET || 'default_secret';
    const decoded = jwt.verify(token, secretKey) as { userId: number };

    const client = await app.pg.connect();
    const selectQuery = `SELECT id, email,created_at FROM userauth WHERE email = ${email}`;
    const { rows } = await client.query(selectQuery, [decoded.userId]);
